// Source: https://flafla2.github.io/2014/08/09/perlinnoise.html
float octave_perlin(vec3 seed, int octaves, float persistence) {
  float total = 0;
  float frequency = 1;
  float amplitude = 1;
  float max_value = 0;

  for (int i = 0; i < octaves; i++) {
    total += (improved_perlin_noise(frequency * seed) * amplitude);
    max_value += amplitude;
    amplitude *= persistence;
    frequency *= 2;
  }

  return total / max_value;
}


mat4 rotate(float theta) {
  return mat4(
    cos(theta),  0, -sin(theta), 0,             // column 1
        0,       1,      0,      0,             // column 2
    sin(theta),  0, cos(theta),  0,             // column 3
        0,       0,      0,      1              // column 4
  );
}


mat4 rotate_light(float theta) {
  return mat4(
    cos(theta), 0, 0, 0,
    0, 1, 0, 0,
    0, 0, sin(theta), 0,
    0, 0, 0, 1
  );
}



// Generate a procedural planet and orbiting moon. Use layers of (improved)
// Perlin noise to generate planetary features such as vegetation, gaseous
// clouds, mountains, valleys, ice caps, rivers, oceans. Don't forget about the
// moon. Use `animation_seconds` in your noise input to create (periodic)
// temporal effects.
//
// Uniforms:
uniform mat4 view;
uniform mat4 proj;
uniform float animation_seconds;
uniform bool is_moon;
// Inputs:
in vec3 sphere_fs_in;
in vec3 normal_fs_in;
in vec4 pos_fs_in;
in vec4 view_pos_fs_in;
// Outputs:
out vec3 color;
// expects: model, blinn_phong, bump_height, bump_position,
// improved_perlin_noise, tangent
void main()
{
  /////////////////////////////////////////////////////////////////////////////
  // Set up the rotating point light
  float theta = animation_seconds * (M_PI / 5.0);
  vec3 pointlight = (view * rotate_light(theta) * vec4(5, 5, 6, 1)).xyz;

  // Get the normal, tangent, and bi-tangent vectors
  vec3 T, B;
  tangent(normalize(sphere_fs_in), T, B);

  // --- Find up the bump mapping and normal mapping. See README.html ---
  mat4 model = model(is_moon, animation_seconds);
  vec3 sp = sphere_fs_in;                         // for notation (sphere point)

  vec3 bp = bump_position(is_moon, sp);           // bump mapping (bump point)
  float epsilon = 0.0001;
  vec3 temp1 = (bump_position(is_moon, sp + epsilon * T) - bp) / epsilon;
  vec3 temp2 = (bump_position(is_moon, sp + epsilon * B) - bp) / epsilon;
  vec3 n = normalize(cross(temp1, temp2));

  // Transform the normal into the camera frame
  n = (transpose(inverse(view)) * transpose(inverse(model)) * vec4(n, 1.0)).xyz;

  // ---- Add noise -----
  // Clouds
  vec3 rot = (rotate_about_y(theta/10) * vec4(bp.y * bp.x, bp.y * bp.y, bp.y * bp.z, 1.0)).xyz;
  // float clouds = 6 * improved_perlin_noise(9 * rot);
  float clouds = 6 * octave_perlin(rot, 4, 10);

  // Lightning on moon
  float lightning = abs(6 * sin(3 * improved_perlin_noise(pointlight)));

  // Add some yellow colour to the land
  float yellow = 4 * octave_perlin(bp, 1, 10);


  // Get the Blinn-Phone shading elements in the camera frame/coordinates
  vec3 p = (view * model * vec4(bp, 1.0)).xyz;                   // point
  vec3 v = -normalize(p);
  vec3 l = normalize(pointlight - p);

  // Set the lighting coefficients
  float phong_exp = 0;
  vec3 ka, kd, ks;
  ka = vec3(0.03);
  if (is_moon) {
    phong_exp = 30;
    kd = mix(vec3(0.1), vec3(0.9, 0., 0.), clouds);
    kd = lightning * kd;
    ks = mix(vec3(1), vec3(0.9, 0., 0.), clouds);
    ks = lightning * ks;
  } else {
    // Check if we're at in valley
    if (bump_height(is_moon, sp) < 0) {
      // Then we have water
      phong_exp = 500;
      kd = mix(vec3(0.2, 0.2, 0.7), vec3(1), clouds);
      ks = vec3(0.8);
    } else {
      // Land
      phong_exp = 5;
      kd = mix(vec3(0.2, 0.7, 0.2), vec3(0.8), clouds);
      kd = mix(kd, vec3(0.97, 0.87, 0.494), yellow);
      ks = vec3(0.1);
    }
  }

  color = blinn_phong(ka, kd, ks, phong_exp, n, v, l);
  /////////////////////////////////////////////////////////////////////////////
}
